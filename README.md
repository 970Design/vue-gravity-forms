...
| `recaptchaKey` | string | No | Deprecated. reCAPTCHA v3 is now configured in the companion WordPress plugin and fetched automatically via the `/recaptcha/config` endpoint. |
| `customComponents` | object | No | Object containing custom field components to override defaults |
| `fieldValues` | object | No | Pre-populate field values keyed by `inputName` |
| `scrollOnSubmit` | boolean | No | Scroll to the confirmation message on success, or to the first error on failure. Defaults to `false`. Use `scroll-padding-top` on the `html` element to account for fixed headers. Use `scroll-behavior` to enable scroll animation. |
